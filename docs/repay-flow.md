...
| `DORKFI_NOT_CONFIGURED` | Pool ABI debt read failed (`get_user` / `get_market` simulation), invalid `marketAppId`, wrong **`assetId`** (e.g. placeholder `456` instead of the ASA id from the debt preview), missing `DORKFI_LENDING_POOL_APP_ID` when the pool differs from `marketAppId`, or ASA metadata could not be loaded. The JSON **`message`** summarizes the cause; **`details`** repeats `poolAppId`, `marketAppId`, `assetId`, `userAddress`, and the raw `DORKFI_LENDING_POOL_APP_ID` env string. |
